...
Bake で CakePHP 標準のテスト雛形を生成する場合は、以下を使います。
...
生成済みの雛形は
[`tests/TestCase/Controller/SystemSettingsControllerTest.php`](tests/TestCase/Controller/SystemSettingsControllerTest.php)
です。`composer test:phpunit` は毎回 `composer init-db` を実行してから PHPUnit を起動します。
現時点では一部 incomplete の雛形で、次のステップで Characterization Test に置き換えます。
